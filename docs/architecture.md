...
Browser workspace
  ...
  ├─ parsed Arazzo model
  ├─ Flow / Chart / Sequence / Docs projections
...
2. Flow, Chart, Sequence, and Docs are projections of that source.
3. Viewer selection may change UI state but must not change execution order.
...
Sequence source is generated by repository code from parsed workflow data.
Mermaid is initialized with `securityLevel: "strict"`. Its generated SVG is
inserted into the page so that diagram messages can remain selectable. This is
an intentional review hotspot in `components/workspace/MermaidView.tsx`.
